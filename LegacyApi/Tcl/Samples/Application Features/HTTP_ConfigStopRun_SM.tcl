...
set logName "simplehttpconfigstoprun"
...
# if the test was configured correctly, then continue on
if {[lindex $::ixTestControllerMonitor 3] == "OK"} {
    puts ""
    puts ">>> Config Applied."
    puts ">>> Running Test..."
    puts ""
    ...
    # if the test didn't stop by itself, then stop it and run again
    if {$::ixTestControllerMonitor == ""} {
	puts ""
	puts ">>> First sample received. Stopping Test..."
	puts ""
	$testController stopRun

	#
	# have the script (v)wait until the test really stops
	#
	vwait ::ixTestControllerMonitor
	puts $::ixTestControllerMonitor
