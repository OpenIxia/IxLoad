...
##########################
    ...
    ${DNSClientActivity} =  Append Item  ${clientActivityList}  protocolAndType=DNS Client
    
    ${serverActivityList} =  Cget  object=${serverCommunity}  field=activityList
    ${DNSServerActivity} =  Append Item  ${serverActivityList}  protocolAndType=DNS Server

#  Client side #    
    ${DNSClientAgent} =  Cget  object=${DNSClientActivity}  field=agent
	${pm} =  Cget  object=${DNSClientAgent}  field=pm
	${dnsConfig} =  Cget  object=${pm}  field=dnsConfig
	${dnsQueries} =  Cget   object=${dnsConfig}  field=dnsQueries
	${makedns} =  Append Item  ${dnsQueries}  commandType=DnsQuery
    Config  object=${makedns}  cmdName=Dns Query  dnsServer=Traffic2_DNSServer1:53
...
##########################
    ...
    ${statNewValue1} =  Get Stat Value  object=${ixLoadStats}  statSource=DNSClient  statName=DNS Total Queries Attempted  timeStamp=latest
    ${statNewValue2} =  Get Stat Value  object=${ixLoadStats}  statSource=DNSClient  statName=DNS Total Queries Sent  timeStamp=latest
	${statNewValue3} =  Get Stat Value  object=${ixLoadStats}  statSource=DNSClient  statName=DNS Total Queries Successful  timeStamp=latest
	${statNewValue4} =  Get Stat Value  object=${ixLoadStats}  statSource=DNSClient  statName=DNS Total Queries Retried  timeStamp=latest
	${statNewValue5} =  Get Stat Value  object=${ixLoadStats}  statSource=DNSClient  statName=DNS Total Queries Failed  timeStamp=latest

	
	
	${statNewValue8} =  Get Stat Value  object=${ixLoadStats}  statSource=DNSServer  statName=DNS Total Queries Received  timeStamp=latest
	${statNewValue9} =  Get Stat Value  object=${ixLoadStats}  statSource=DNSServer  statName=DNS Total Queries Responded Successfully  timeStamp=latest
	${statNewValue10} =  Get Stat Value  object=${ixLoadStats}  statSource=DNSServer  statName=DNS Total Queries Failed  timeStamp=latest
	
	
	
	Log To Console 	DNS Total Queries Attempted=${statNewValue1}
	Log To Console 	DNS Total Queries Sent=${statNewValue2}
	Log To Console  DNS Total Queries Successfu=${statNewValue3}
	Log To Console  DNS Total Queries Retried=${statNewValue4}
	Log To Console  DNS Total Queries Failed=${statNewValue5}
	...
	Run Keyword If  '${statNewValue5}' != '0'  FAIL  '${statNewValue5}'"Queries Failed !"  ELSE  Log To Console  "Queries Failed stat check done." 
